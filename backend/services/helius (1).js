...
export async function fetchWalletTxns(walletAddress, beforeSignature = null, limit = 100, type = null) {
...
// Keep old name as alias for console commands
export const fetchWalletSwaps = fetchWalletTxns;

/**
 * Two-pass approach:
 * Pass 1: type=SWAP — efficiently pulls Jupiter/Raydium swaps (most pages are actual swaps)
 * Pass 2: no type filter — catches Pump.fun and other non-SWAP trades
 * Deduplicates by tx signature
 */
export async function fetchAllSwaps(walletAddress) {
  const seen = new Set();
  const allTrades = [];

  // Pass 1: SWAP type only — very efficient, almost every tx is a real swap
  console.log('Pass 1: Fetching SWAP transactions...');
  let beforeSig = null;
  for (let page = 0; page < 100; page++) {
    const txns = await fetchWalletTxns(walletAddress, beforeSig, 100, 'SWAP');
    ...
    console.log(`  SWAP page ${page + 1}: ${allTrades.length} total swaps`);
  }

  // Pass 2: ALL types — catches Pump.fun transfers that aren't tagged as SWAP
  console.log('Pass 2: Fetching all transactions for Pump.fun...');
  beforeSig = null;
  for (let page = 0; page < 200; page++) {
    const txns = await fetchWalletTxns(walletAddress, beforeSig, 100);
    if (!txns || txns.length === 0) break;
    ...
    if (page % 10 === 0) {
      console.log(`  ALL page ${page + 1}: ${allTrades.length} total swaps`);
    }
  }

  console.log(`Total swaps found: ${allTrades.length}`);
  return allTrades;
}
